 ...
 * @type {Object}
 */
enchant.ui = { assets: ['pad.png', 'apad.png', 'item.png', 'font0.png'] };
...
enchant.ui.LifeLabel = enchant.Class.create(enchant.Group, {
    ...
    initialize: function(x, y, maxlife) {
        ...
        this.x = x || 0;
        this.y = y || 0;
        this._maxlife = maxlife || 9;   // 指定なしの場合9
        this._life = 0;
        this.label = new enchant.ui.MutableText(0, 0, 100);
        this.label.text = 'LIFE :';
        this.addChild(this.label);
        this.heart = [];
        for (var i = 0; i < this._maxlife; i++) {
            this.heart[i] = new enchant.Sprite(16, 16);
            this.heart[i].image = enchant.Game.instance.assets['item.png'];
...
enchant.ui.SpeedLabel = enchant.Class.create(enchant.Group, {
    /**
     * スピード表示用にLifeLabelクラスを流用したもの
     * item.png 内の青宝石画像を用いる
     * @constructs
     * @param x
     * @param y
     * @param maxspd
     */
    initialize: function(x, y, maxspd) {
        enchant.Group.call(this);
        this.x = x || 0;
        this.y = y || 0;
        this._maxspd = maxspd || 9;   // 指定なしの場合9
        this._spd = 0;
        this.label = new enchant.ui.MutableText(0, 0, 70);
        this.label.text = 'SPD:';
        this.addChild(this.label);
        this.hexa = [];
        for (var i = 0; i < this._maxspd; i++) {
            this.hexa[i] = new enchant.Sprite(16, 16);
            this.hexa[i].image = enchant.Game.instance.assets['item.png'];
            this.hexa[i].x = this.label.width + i * 16;
            this.hexa[i].y = -3;
            this.hexa[i].frame = 0;
            this.addChild(this.hexa[i]);
        }
    },
    /**
     * 現在スピード数
    ...
    spd: {
        ...
        set: function(newspd) {
            this._spd = newspd;
            if (this._maxspd < newspd) {
                this._spd = this._maxspd;
            }
            for (var i = 0; i < this._maxspd; i++) {
                this.hexa[i].visible = (i <= newspd - 1);
            }
        }
    } 
});
...
enchant.ui.ShotLabel = enchant.Class.create(enchant.Group, {
    /**
     * ショットLv表示用にLifeLabelクラスを流用したもの
     * item.png 内の赤宝石画像を用いる
     * @constructs
     * @param x
     * @param y
     * @param maxshotLv
     */
    initialize: function(x, y, maxshotLv) {
        enchant.Group.call(this);
        this.x = x || 0;
        this.y = y || 0;
        this._maxshotLv = maxshotLv || 9;   // 指定なしの場合9
        this._shotLv = 0;
        this.label = new enchant.ui.MutableText(0, 0, 80);
        this.label.text = 'SHOT:';
        this.addChild(this.label);
        this.octa = [];
        for (var i = 0; i < this._maxshotLv; i++) {
            this.octa[i] = new enchant.Sprite(16, 16);
            this.octa[i].image = enchant.Game.instance.assets['item.png'];
            this.octa[i].x = this.label.width + i * 16;
            this.octa[i].y = -3;
            this.octa[i].frame = 1;
            this.addChild(this.octa[i]);
        }
    },
    /**
     * 現在ショットLv
    ...
    shotLv: {
        ...
        set: function(newshotLv) {
            this._shotLv = newshotLv;
            if (this._maxshotLv < newshotLv) {
                this._shotLv = this._maxshotLv;
            }
            for (var i = 0; i < this._maxshotLv; i++) {
                this.octa[i].visible = (i <= newshotLv - 1);
            }
        }
    }
});
...
enchant.ui.BombLabel = enchant.Class.create(enchant.Group, {
    /**
     * ボム表示用にLifeLabelクラスを流用したもの
     * item.png 内の爆弾画像を用いる
     * @constructs
     * @param x
     * @param y
     * @param maxbomb
     */
    initialize: function(x, y, maxbomb) {
        enchant.Group.call(this);
        this.x = x || 0;
        this.y = y || 0;
        this._maxbomb = maxbomb || 9;   // 指定なしの場合9
        this._bomb = 0;
        this.label = new enchant.ui.MutableText(0, 0, 100);
        this.label.text = 'BOMB :';
        this.addChild(this.label);
        this.hexa = [];
        for (var i = 0; i < this._maxbomb; i++) {
            this.hexa[i] = new enchant.Sprite(16, 16);
            this.hexa[i].image = enchant.Game.instance.assets['item.png'];
            this.hexa[i].x = this.label.width + i * 16;
            this.hexa[i].y = -3;
    ...
    bomb: {
        ...
        set: function(newbomb) {
            this._bomb = newbomb;
            if (this._maxbomb < newbomb) {
                this._bomb = this._maxbomb;
            }
            for (var i = 0; i < this._maxbomb; i++) {
                this.hexa[i].visible = (i <= newbomb - 1);
            }
        }
    } 
});
...
enchant.ui.Bar = enchant.Class.create(enchant.Sprite, {
     ...
     */
    initialize: function(x, y) {
        enchant.Sprite.call(this, 1, 3);
        this.image = new enchant.Surface(1, 3);// Null用
        this.image.context.fillColor = 'RGB(0, 0, 256)';
        this.image.context.fillRect(0, 0, 1, 3);
